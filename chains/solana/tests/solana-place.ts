...
describe("solana-place", () => {
  ...
  it("initialize game", async () => {
    const gameKeypair = await createGameAccount(
      program,
      programProvider,
      gameAuthority,
      1,
  ...
  changeColorsParams.forEach((payerIsGameAuthority) => {
    const testName =
      "change colors" +
      (payerIsGameAuthority ? " with game authority payer" : "");
    it(testName, async () => {
      ...
      let gameState = await program.account.gameAccount.fetch(
        gameKeypair.publicKey
      );
      let expectedBoard = emptyBoard(
        gameAuthority,
        boardHeight,
        boardWidth,
        changeCost
      ...
      const changeColorRequests: ChangeColorRequest[] = new Array(150)
        .fill(0)
        .map((_, index) => ({ row: index, column: index, color: index }));

      const [event] = await catchEvent(
        program,
      ...
      gameState = await program.account.gameAccount.fetch(
        gameKeypair.publicKey
      );
      expectedBoard = changeColorRequests.reduce(
        (board, request) =>
          changeColor(board, request.row, request.column, request.color),
        expectedBoard
      );
      ...
      const expectedChange = payerIsGameAuthority
        ? transactionFee
        : transactionFee.add(
            new BN(changeCost * 1000 * changeColorRequests.length)
          );
      ...
      expect(actualChange.sub(expectedChange).abs().lte(new BN(10000))).to.be
        .true;
    });
  });

  it("error: pixel is not withing game's bounds", async () => {
    let gameKeypair = await createGameAccount(
      program,
      programProvider,
      gameAuthority,
      5,
    ...
    try {
      await program.methods
        .changeColors(
          encodeChangeColorRequests([{ row: 10, column: 10, color: 42 }])
        )
        .accounts({
          gameAccount: gameKeypair.publicKey,

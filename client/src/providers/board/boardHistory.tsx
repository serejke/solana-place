...
import {
  ...
  boardHistoryReducer,
} from "../../reducers/boardHistoryReducer";
...
export function BoardHistoryProvider({
  ...
  const [boardHistory, boardHistoryDispatch]: [BoardHistory, SetState] =
    useReducer(boardHistoryReducer, { events: [] });
...
export function useBoardHistoryDispatch(): BoardHistoryDispatch {
  ...
  if (!dispatch) {
    throw new Error(
      `useBoardHistoryDispatch must be used within a BoardHistoryProvider`
    );
  }
  return dispatch[1];
}

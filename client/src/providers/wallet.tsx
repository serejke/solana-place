...
export function WalletProvider({ children }: { children: React.ReactNode }) {
  ...
  return (
    <SolanaWalletProvider
      wallets={wallets}
      onError={(e) =>
        addNotification(buildErrorNotification("Wallet was not connected", e))
      }
      autoConnect
    >
...
function WalletModalProviderWithListener({
  children,
}: {
  ...
  React.useEffect(() => {
    if (wallet.connecting) {
      addNotification(
        buildInfoNotification("Connecting wallet", {
          type: "string",
  ...
  React.useEffect(() => {
    if (wallet.connected) {
      addNotification(
        buildSuccessNotification("Wallet connected", {
          type: "string",

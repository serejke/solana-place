...
export function useColorPickerStateAndActions(): ColorPickerProviderState {
  ...
  if (!state) {
    throw new Error(`useColorPickerState must be used within a ColorPickerProvider`);
  }
  const [colorPickerPosition, setColorPickerPosition] = state;
  const openColorPicker = React.useCallback((colorPickerPosition: ColorPickerPosition) => {

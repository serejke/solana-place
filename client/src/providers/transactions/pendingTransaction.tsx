...
export function usePendingTransaction(): State {
  ...
  if (!context) {
    throw new Error(
      `usePendingTransaction must be used within a PendingTransactionProvider`
    );
  }
  return context[0];
}
...
function useSetPendingTransactionState(): SetState {
  ...
  if (!context) {
    throw new Error(
      `useSetPendingTransaction must be used within a PendingTransactionProvider`
    );
  }
  return context[1];
}
...
export function useAddOrDeleteChangedPixel(): AddOrDeleteChangedPixel {
  ...
  const addChangedPixel = React.useCallback(
    (changedPixel: ChangedPixel) => {
      setPendingTransactionState((state) => {
        const coordinates = changedPixel.coordinates;
        const index = state.changedPixels.findIndex((pixel) =>
          areEqual(coordinates, pixel.coordinates)
        );
  ...
  const deleteChangedPixel = React.useCallback(
    (pixelCoordinates: PixelCoordinates) => {
      setPendingTransactionState((state) => ({
        ...state,
        changedPixels: state.changedPixels.filter(
          (changed) => !areEqual(changed.coordinates, pixelCoordinates)
        ),
      }));
    },
    [setPendingTransactionState]
...
export function useSetAndUnsetPendingTransaction(): SetAndUnsetPendingTransaction {
  ...
  const unsetPendingTransaction = React.useCallback(
    (
      transactionSignature: TransactionSignature,
      clearChangedPixels: boolean
    ) => {
  ...
  const setPendingTransaction = React.useCallback(
      ...
      const pendingTransactionIntervalId = window.setInterval(async () => {
        ...
        if (processResult === "dropped") {
          if (await unsetPendingTransaction(pendingTransaction, false)) {
            addNotification(
              buildWarnNotification("Transaction has been dropped", {
...
export function createTransactionConfirmedNotification() {
  return buildSuccessNotification("Transaction has been confirmed", {
    type: "string",
...
async function processPendingTransaction(
  ...
  const transactionChanges = await fetchTransactionEvents(
    pendingTransactionSignature
  );
  if (transactionChanges === null) {
    console.log(
      `Transaction ${pendingTransactionSignature} is not confirmed yet, keep waiting...`
    );
    return "retry";
  }
  for (const gameEventWithTransactionDetails of transactionChanges.events) {
    updateBoardStateAndHistory(

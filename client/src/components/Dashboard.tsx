...
import {
  useBoardConfig,
  useSetBoardConfig,
} from "../providers/board/boardConfig";
  ...
  return (
    <div className="dashboard-item">
      <div className="dashboard-cancel-draggable">
        <WalletMultiButton
          className={`action-button ${isPhone ? "action-button-phone" : ""}`}
        >
          {content}
        </WalletMultiButton>
      </div>
...
function SendChangesButton() {
  ...
  const isDisabled =
    !isWalletConnected || !isAnyChanged || isPendingTransaction;
  ...
  const onClick = React.useCallback(() => {
    ...
    changePixels(changedPixels, wallet)
      .then((transactionSignature) => {
        ...
        addNotification(
          buildInfoNotification("Transaction has been sent", {
      ...
      .catch((e) =>
        addNotification(buildErrorNotification("Failed to change pixels", e))
      );
  }, [
    isDisabled,
    changedPixels,
    ...
    <div className="dashboard-item">
      ...
      >
        {isPendingTransaction ? (
          ...
          <PaperAirplaneIcon
            className={`send-changes-icon send-changes-icon-${
...
function ShowGridToggle() {
  ...
  return (
    <div className="dashboard-item">
      <div
        className={`dashboard-cancel-draggable dashboard-icon-holder ${
          isChecked ? "checked-toggle" : ""
        }`}
        data-tip={true}
        data-for="grid-tooltip-id"
      >
        <Squares2X2Icon className="grid-icon" onClick={onClick} />
      </div>
      <ReactTooltip
        className="dashboard-tooltip"
        id="grid-tooltip-id"
...
function ShowHistoryToggle() {
  ...
  return (
    <div className="dashboard-item">
      ...
      >
        <HistoryIconNotChecked className="history-icon" onClick={onClick} />
      </div>
      <ReactTooltip
        className="dashboard-tooltip"
...
function OnlineStatus() {
  ...
  return (
    <div className="dashboard-item">
      ...
      <ReactTooltip
        class={`dashboard-tooltip online-status-tooltip online-status-tooltip-${
          isOnline ? "success" : "connecting"
        }`}
  ...
  return (
    <div className="dashboard-item">
      <InformationCircleIcon
        ...
        onClick={() =>
          setAbout((prevState) => ({ ...prevState, showAboutModal: true }))
        }
      />
    </div>
  );
  ...
  return (
    <div className="board-history">
      <table className="board-history-table">
        ...
        <tbody>
          {boardRows.map(
            ({ row, column, oldColor, newColor, transactionDetails }) => (
              <tr
              ...
              >
                ...
                <td>
                  {shortenTransactionSignature(transactionDetails.signature)}
                  {SHORTENED_SYMBOL}
                </td>
                <td>
...
function formatTime(timestamp: number): string {
  ...
  const elapsedSeconds = Math.abs(
    Math.floor(date.getTime() / 1000) - timestamp
  );
...
function BoardHistoryChangeArrow({
  oldColor,
  newColor,
}: {
  oldColor: number;
  ...
  return (
    <div className="board-history-cell">
      <div
        className="board-history-pixel-plate"

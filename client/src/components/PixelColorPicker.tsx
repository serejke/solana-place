...
export function PixelColorPicker() {
  ...
  const { colorPickerPosition, closeColorPicker } =
    useColorPickerStateAndActions();

  const onColorPicked = React.useCallback(
    (color: ColorResult) => {
  ...
  const popover: CSSProperties = {
    ...
    zIndex: 2,
    left:
      colorPickerPosition.popupPosition.clientX -
      GITHUB_PICKER_TRIANGLE_SIZE / 2,
    top:
      colorPickerPosition.popupPosition.clientY +
      GITHUB_PICKER_TRIANGLE_SIZE / 2,
  };

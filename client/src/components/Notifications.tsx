  ...
  return (
    <div className="notifications">
      {notifications.map((notification) => (
        <NotificationPlate key={notification.id} notification={notification} />
      ))}
    </div>
  );
}
...
function NotificationPlate({ notification }: NotificationPlateProps) {
  ...
  switch (content.type) {
    ...
    case "transactionWasDropped":
      contentToRender = (
        <TransactionWasDroppedNotification
    ...
    case "maximumChangesReached":
      contentToRender = (
        <MaximumChangesReachedNotification limit={content.limit} />
      );
      break;
  }
  return (
...
function TransactionWasDroppedNotification({
  ...
  return (
    <div>
      Transaction {shortenTransactionSignature(transactionSignature)} has not
      been confirmed in {timeout / 1000} seconds and has probably been dropped
      by the cluster. Try sending it again...
      <ExplorerTransactionLink
        signature={transactionSignature}
        className="link-icon-center-aligned"
      />
    </div>
  );
}

function WaitingForTransactionNotification({
  transactionSignature,
}: {
  transactionSignature: TransactionSignature;
}) {
  return (
    <div>
      ...
      {shortenTransactionSignature(transactionSignature)} to confirm
      {SHORTENED_SYMBOL}
      <ExplorerTransactionLink
        signature={transactionSignature}
        className="link-icon-center-aligned"
...
function MaximumChangesReachedNotification({ limit }: { limit: number }) {
  ...
  const onClick = React.useCallback(() => {
    ...
    setNotifications((notifications) =>
      notifications.filter(
        (notification) => notification.content.type !== "maximumChangesReached"
      )
    );
  ...
  return (
    <div>
      ...
      <span className="link-span" onClick={onClick}>
        Clear all?
      </span>
    </div>
  );

...
import {useAddNotification, useNotificationsState} from "../../providers/notifications/notifications";
import {useCurrentPixelColorState} from "../../providers/color/currentColor";

export type ChangeOrCancelPixelAction = (pixelCoordinates: PixelCoordinates, isDrawingMode: boolean) => void;
...
export function useChangeOrCancelPixel(): ChangeOrCancelPixelAction {
  ...
  return React.useCallback((pixelCoordinates: PixelCoordinates, isDrawingMode) => {
    const isAlreadyChanged = changedPixels.some(changed => areEqual(pixelCoordinates, changed.coordinates));
    ...
    if (changedPixels.length === MAX_CHANGES_PER_TRANSACTION) {
      ...
      const isAlreadyShowing = notifications.some(notification => notification.title === notificationTitle);
      if (!isAlreadyShowing) {

...
function useRequestInitialBoardState() {
  ...
  React.useEffect(() => {
    fetchBoard(serverUrl).then((boardStateDto) => {
      boardDispatch({
        type: "initialState",
...
function useRequestInitialBoardHistory() {
  ...
  React.useEffect(() => {
    fetchBoardHistory(serverUrl).then((boardHistory) => {
...
function useSubscribeToBoardEvents() {
  ...
  const messageHandler = React.useCallback(
    async (message: any) => {
      const gameEventWithTransactionDetails =
        parseGameEventWithTransactionDetailsFromDto(message);
      if (gameEventWithTransactionDetails) {
        ...
        const transactionSignature =
          gameEventWithTransactionDetails.transactionDetails.signature;
...
export function updateBoardStateAndHistory(
  ...
  boardDispatch({
    type: "updatePixels",
    updatedPixels: gameEventWithTransactionDetails.event.changes.map(
      ({ row, column, newColor }) => ({
